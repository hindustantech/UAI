...
export async function generateSalarySlipPDF(payroll, filePath) {
    return new Promise((resolve, reject) => {
        ...
        doc.font("Helvetica").fontSize(9)
           .text(
               `SALARY SLIP  —  ${per.label ?? ""}   |   Pay Date: ${payroll.payDate ? new Date(payroll.payDate).toLocaleDateString("en-IN") : ""}`,
        ...
        /* ───── LOSS OF PAY (LOP) INFO BAR ───── */
        const lopDays   = lop.lopDays   ?? 0;
        const lopAmount = lop.lopAmount ?? 0;

        doc.rect(LEFT, y, PAGE_W, 20).fill("#FFF3E0");   // orange tint
        doc.fillColor(ORANGE).font("Helvetica-Bold").fontSize(8)
           .text(
               `Loss of Pay (LOP):  ${lopDays} absent day(s)  ×  per-day rate  =  ${INR(lopAmount)} deducted`,
               LEFT + 5, y + 6, { width: PAGE_W - 10 }
           );
        y += 28;
        ...
        const dedRows = [
            ...
            // LOP always shown (even if 0, for transparency)
            [`Loss of Pay (${lopDays} days)`, lopAmount],
        ].filter(([, v]) => (v ?? 0) > 0);

        const maxR = Math.max(earnRows.length, dedRows.length);

        const drawRow = (label, val, x, w, rowY, i, isTotal = false, isLOP = false) => {
            let bg;
            if (isTotal)    bg = (x === earX ? "#C8E6C9" : "#FFCDD2");
            else if (isLOP) bg = "#FFF3E0";   // orange tint for LOP row
            else            bg = i % 2 === 0 ? WHITE : LGRAY;

            doc.rect(x, rowY, w, rowH).fill(bg);

            const color = isTotal ? (x === earX ? GREEN : RED)
                        : isLOP  ? ORANGE
                        : "#333333";
            const fw = (isTotal || isLOP) ? "Helvetica-Bold" : "Helvetica";
        ...
        for (let i = 0; i < maxR; i++) {
            ...
            if (dedRows[i]) {
                const isLOPRow = dedRows[i][0].startsWith("Loss of Pay");
                drawRow(dedRows[i][0], dedRows[i][1], dedX, dedW, rowY, i, false, isLOPRow);
            } else {
                doc.rect(dedX, rowY, dedW, rowH).fill(i % 2 === 0 ? WHITE : LGRAY);
            }
        }
        y += maxR * rowH;

        // Gross + Total Deductions totals row
